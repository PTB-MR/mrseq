...
CamNrDynamics = len(parameters['enumerate_coeff'])*len(parameters['SLICE_POS'])*len(parameters['GRAD'])*len(parameters['RISE_TIMES'])*parameters['N_AVG']
...
trig = pp.make_digital_output_pulse(channel="ext1", duration=parameters['trig_duration'], delay=0) # creating the MFC trigger pulse 
...
# ADC Block
adc = pp.make_adc(num_samples=adc_num_samples, duration=parameters['adc_duration'], system=system, delay=system.adc_dead_time) # Analog Digital Converter
...
for avg in range(parameters['N_AVG']):
    ...
    for idx_rise_time, rise_time in enumerate(parameters['RISE_TIMES']):
        ...
        rise_time = np.round(rise_time,decimals=6) # Not rounding the rise time here causes an error during amplitude calculation
        ...
        for idx_grad, grad in enumerate(parameters['GRAD']): 
            ...
            for idx_slice_pos, slice_pos in enumerate(parameters['SLICE_POS']): 
                ...
                for idx_amp_fac, amp_fac in enumerate(parameters['enumerate_coeff']):
                    ...
                    label_contents = [avg_label, seg_label, grad_label, slice_label, amp_fac_label] # All labels, average, segment, gradient, slice and amp_factor
                    ...
                    if amp_fac != 0: # If there is an amp (if gradient present)
                        ...
                        g_triangle = pp.make_trapezoid(channel=grad, system=system, rise_time=rise_time, flat_time=0, amplitude=amp, delay=0) 
...
seq.set_definition("Acquisiton Method: ", method)
#
